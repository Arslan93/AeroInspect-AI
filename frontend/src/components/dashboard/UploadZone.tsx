...
import { uploadInspection } from "@/lib/uploadInspection";
  ...
  for (const file of Array.from(files)) {
    ...
    try {
      ...
      newFiles.push({
        ...
        ? ["Potential structural anomaly"]
        : data.result?.defects || [],

        severity:
        file.type.startsWith("video")
        ? "MEDIUM"
        : data.result?.severity || "LOW",
        report: data.report,
        });
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground mt-1 font-mono">JPG · PNG · MP4 · WEBM · Drone footage supported</div>
          </div>
        </motion.div>
